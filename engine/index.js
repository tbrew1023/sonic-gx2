...
sfxObj2.death.src = "res/sfx/SonicMania/Global/hurt.wav";
sfxObj2.explosion.src = "res/sfx/Explosion.wav";
sfxObj2.jump.src = "res/sfx/jump.wav"; 
sfxObj2.looseRings.src = "res/sfx/SonicMania/Global/loseRings.wav";
sfxObj2.ring.src = "res/sfx/SonicMania/Global/Ring.wav";
sfxObj2.spindash.src = "res/sfx/SonicMania/Global/Roll.wav";
sfxObj2.spring2.src = "res/sfx/SonicMania/Global/Spring.wav";
...
var sonic = {
	...
	char: {
		...
		//constants (only change if you know what you are doing)
		ACC: 0.06,	// acceleration
		DEC: 0.7,		// deceleration
		FRC: 0.046875,	// friction
		TOP: 6,			// top speed (pixels/frame)
		JMP: 6.5,		// jump speed
		GRV: 0.21875,	// gravity
...
function controls() {
	...
	if (keysDown[32] && char.state != -1 && keysDown[40] != true && char.pJump != true) {//jumping
		char.state = -1;
		char.anim = anim.jump;
	...
	if (!keysDown[32] && char.state != -1) {
		char.pJump = false;
	}
	if (!keysDown[32] && char.state == -1) { // reset drop dash if you release the jump button
		char.dropCharge = 0;
		char.pDropDash = false;
	}
	if (char.state == -1 && keysDown[32] != true && char.jumpState == 1 && char.yv < -4) { // controllable jump height
		char.yv = -4;
	}
	...
	if (keysDown[32] == true && char.pDropDash == true && char.state == -1) { // charge the drop dash
		char.dropCharge += 1;
	}
			...
			if (gamepads[i] != undefined) {// The only reason why I need this line is because of Chrome. I hate you too, Chrome.
				if (gamepads[i].buttons[0].pressed) {
					if (keysDown[32] == false) {
						...
						controlPressed({ keyCode:32 });
					}
					keysDown[32] = true;
				}
				else {
					if (keysDown[32] == true) {
						controlReleased({ keyCode: 32 });
					}
					keysDown[32] = false;
				}
...
function controlPressed(e) {
	if (char.homing == true && char.state == -1) {
		if (e.keyCode == 32 && char.pHoming == false && keysDown[32] == false) {
			char.pHoming = true;
			char.currentAnim = anim.jump;
			char.jumpState = 1;
			var result = [0, 0, 200];
	...
	if (char.levitate == true && char.state == -1 && char.jumpState == 1 && char.levTimer > 0) {
		if (e.keyCode == 32 && keysDown[32] == false) {
			char.pHoming = true;
...
function controlReleased(e) {
	if (keysDown[32] == true && e.keyCode == 32) {
...
window.addEventListener("keydown", function (e) {
	...
	if (e.keyCode == 32 && char.rolling && char.currentAnim == anim.spindash) {
		// sfx.src = "";
		// sfx.src = sfxObj.spindash;
		// sfx.currentTime = 0;
		sfxObj2.spindash.load();
